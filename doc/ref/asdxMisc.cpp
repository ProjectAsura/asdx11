...
{
    HRESULT hr = S_OK;
    ID3D11ShaderReflection* pVSR = nullptr;
...
{
    // NULLチェック.
    if ( pDeviceContext == nullptr  || pBuffer == nullptr )
    { return false; }
...
std::wstring GetDirectoryPath( const char16* filePath )
{
    ...
    {
        std::wstring result = path.substr( 0, idx + 1 );
        return result;
    }
...
//------------------------------------------------------------------------------------------------
//      文字列から最後のディレクトリパスをそぎ落とします.
//-----------------------------------------------------------------------------------------------
bool CutLastDirectoryPath( char16* path )
{
    return PathRemoveFileSpecW( path ) == TRUE;
}
